...
in {
  options.modules.desktop.termEmu.alacritty = with types; {
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      programs.kitty.font.name = "JetBrainsMonoMedium Nerd Font";
    ...
    (mkIf cfg.activePalette == "ayu-dark" {
      programs.kitty.settings.include = "${configDir}/kitty/ayu-dark.conf";
    })

    (mkIf cfg.activePalette == "onedark" {
      programs.kitty.settings.include = "${configDir}/kitty/onedark.conf";
    })
  ]);
}
