...
in {
  ...
  config = mkIf cfg.enable {
    homeManager.programs.vscode = {
      ...
      userSettings = let
        themeCfg =
          trivial.importJSON "${themesDir}/config/vscodium/themeCfg.json";
        vscCfg = trivial.importJSON "${codeDir}/settings.json";
      in [ vscCfg themeCfg ];
      keybindings = trivial.importJSON "${codeDir}/keybindings.json";
    };
  };
}
