...
main = do
  ...
  let
    myCPU =
      deocrateWithSetClassAndBoxes "cpu" $ pollingGraphNew cpuCfg 5 cpuCallback
    myMem =
      deocrateWithSetClassAndBoxes "mem" $ pollingGraphNew memCfg 5 memCallback
    myNet = deocrateWithSetClassAndBoxes "net" $ networkGraphNew netCfg Nothing
    myLayout =
      deocrateWithSetClassAndBoxes "layout" $ layoutNew defaultLayoutConfig
    myWindows =
      deocrateWithSetClassAndBoxes "windows" $ windowsNew defaultWindowsConfig
    myWorkspaces = flip widgetSetClassGI "workspaces" =<< workspacesNew
      defaultWorkspacesConfig
        ...
        , getWindowIconPixbuf         =
          ...
          <|||> (\size _ ->
                  lift $ loadPixbufByName size "application-default-icon"
                )
        , widgetGap                   = 0
    ...
    myLabelSetter workspace = return $ case workspaceName workspace of
      "1" -> "一"
      "2" -> "二"
      "3" -> "三"
      "4" -> "四"
    ...
    myMpris = mpris2NewWithConfig MPRIS2Config
    ...
    selectedConfig = fromMaybe baseConfig $ lookup
      hostName
